...
import { copyBuildingModuleAction } from "@/app/(protected)/modules/actions";
import { AppModal } from "@/app/components/app-modal";
import { ModalFormActions } from "@/app/components/modal-form-actions";
import { useOptionalNavigationLoading } from "@/app/components/navigation-loading-context";
...
type ModuleCopyModalProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  module: BuildingModuleSummary;
};

export function ModuleCopyModal({
  open,
  onOpenChange,
  module,
}: ModuleCopyModalProps) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await copyBuildingModuleAction({
        sourceId: module.id,
        name: name.trim(),
        note: note.trim(),
      });
      ...
      const moduleHref = `/modules/${result.id}`;
      handleOpenChange(false);
      navigationLoading?.beginNavigation(
        moduleHref,
        t("modules.loading", "Ielādē moduli…"),
      );
  ...
  return (
    <AppModal
      ...
      title={t("modules.copy.title", "Kopēt moduli")}
      description={t(
        "modules.copy.description",
        "Ievadi jaunā moduļa nosaukumu. Dati tiks nokopēti no izvēlētā moduļa.",
      )}
      blocking={isPending}
      dirty={name !== initialName || note !== initialNote}
    >
      <form noValidate onSubmit={handleSubmit} className="space-y-4">
        <label htmlFor="module-copy-name" className="block">
          <span className="mb-1.5 block text-sm font-medium text-zinc-700">
            {t("common.name", "Nosaukums")}
          </span>
          <input
            id="module-copy-name"
            name="module-copy-name"
            type="text"
            value={name}
            onChange={(event) => {
            ...
            aria-describedby={nameError ? "module-copy-name-error" : undefined}
          />
          {nameError ? (
            <p id="module-copy-name-error" className="mt-1 text-sm text-red-600" role="alert">
              {nameError}
            </p>
          ) : null}
        </label>

        <label htmlFor="module-copy-note" className="block">
          <span className="mb-1.5 block text-sm font-medium text-zinc-700">
            {t("common.note", "Piezīme")}
          </span>
          <input
            id="module-copy-note"
            name="module-copy-note"
            type="text"
            value={note}
            maxLength={255}
            ...
            aria-describedby={noteError ? "module-copy-note-error" : undefined}
          />
          {noteError ? (
            <p id="module-copy-note-error" className="mt-1 text-sm text-red-600" role="alert">
              {noteError}
            </p>
          ) : null}
        </label>
