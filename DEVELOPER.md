...
- **Laika grafiks** (`/timeline-graph`, `module_timeline_graph`) — projekti (`active` / `approved` / `completed`) secībā no `company_timeline_graph_order` (DnD prioritāte); darbietilpība no tāmes (apjoms × laika norma), rādīta kā pilnas dienas + stundas (`1 d = 8 c/h`); plānošana pa darba dienām (sestdiena/svētdiena izlaistas); sticky projekta kolonna + ritināms kalendārs; sakļaujams projekts / kategorija (subkategorijas); neapstiprinātajiem — aptuvena/muted josla; reorder caur `reorderTimelineGraphProjectsAction`
...
- [x] Darbinieki, instrumenti, termiņu un laika grafiks — `/workers`, `/tools`, `/timeline`, `/timeline-graph` ar frontend moduļu gating, atsevišķām pārvaldības tiesībām un DB migrācijām (`141`–`148`)
