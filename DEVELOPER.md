...
- **Laika grafiks** (`/timeline-graph`, `module_timeline_graph`) — projekti (`active` / `approved` / `completed`) secībā no `company_timeline_graph_order` (DnD prioritāte ar `timeline_graph.manage`; bez tiesības — tikai skatīt); darbietilpība no tāmes (apjoms × laika norma), rādīta kā pilnas dienas + stundas (`1 d = 8 c/h`); plānošana pa darba dienām (sestdiena/svētdiena izlaistas); sticky projekta kolonna + ritināms kalendārs; sakļaujams projekts / kategorija (subkategorijas); neapstiprinātajiem — aptuvena/muted josla; reorder caur `reorderTimelineGraphProjectsAction`
...
│   ├── timeline-graph/ # Labor workload schedule: types, repository/order, build-sections, workday schedule
...
- [x] Darbinieki, instrumenti un laika grafiks — `/workers`, `/tools`, `/timeline-graph` ar frontend moduļu gating, atsevišķām pārvaldības tiesībām (`timeline_graph.manage`) un DB migrācijām (`141`–`149`); `module_timeline` / Termiņu grafiks noņemts (`154`); `module_timeline_graph` DB atjaunots (`155`)
