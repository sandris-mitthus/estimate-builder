...
- **Termiņu grafiks** (`/timeline`, `module_timeline`) — apstiprinātie un pabeigtie projekti tiek sinhronizēti uz `company_timeline_entries` un rādīti sarkanā grafika joslā; sākuma/beigu datumi nāk no tāmes datuma/termiņa un ir labojami ar `timeline.manage`
...
│   ├── timeline/       # Approved-project deadline timeline (Termiņu grafiks) + schedule actions
...
- [x] Darbinieki, instrumenti, termiņu un laika grafiks — `/workers`, `/tools`, `/timeline`, `/timeline-graph` ar frontend moduļu gating, atsevišķām pārvaldības tiesībām (`timeline_graph.manage`) un DB migrācijām (`141`–`149`)
...
- [x] Moduļa lielumi — **Dzīvojamā platība** (m²), logu/durvju **kopējais perimetrs**, vairākas vienlaikus atvērtas sadaļas apjoma izvēlē un sagataves piemēra piezīme bez moduļa nosaukuma
